...
SAML.prototype.generateAuthorizeRequest = function (req, isPassive, callback) {
  ...
  function finishGenerateAuthReq(){
    ...
    return callback(null, xmlbuilder.create(request).end());
  }

  if(self.options.validateInResponseTo) {
      self.cacheProvider.save(id, instant, function(err, val){
      if(err){
        return callback('Unable to save to cacheProvider: ' + err, null);
      }

      return finishGenerateAuthReq();
    });
  }
  else
  {
...
SAML.prototype.generateLogoutRequest = function (req) {
  ...
  var request = {
    'samlp:LogoutRequest' : {
      ...
      '@Version': '2.0',
      '@IssueInstant': instant,
      '@Destination': this.options.logoutUrl || this.options.entryPoint,
      'saml:Issuer' : {
        '@xmlns:saml': 'urn:oasis:names:tc:SAML:2.0:assertion',
...
SAML.prototype.getAuthorizeUrl = function (req, callback) {
  ...
  this.generateAuthorizeRequest(req, this.options.passive, function(err, request){
    if(err){
...
SAML.prototype.validatePostResponse = function (container, callback) {
  ...
  var doc = new xmldom.DOMParser().parseFromString(xml);
    ...
    var finishValidatingPostResponse = function(){
  ...
  if(self.options.validateInResponseTo && inResponseTo){
        self.cacheProvider.get(inResponseTo, function(err, value){
            if (err) {
                return callback(new Error('Error retrieving inResponseTo value from cacheProvider: ' + err), null, false);
            }
            if (!value) {
                return callback(new Error('InResponseTo is not valid'), null, false);
            }

            finishValidatingPostResponse();
    });
  }
  else
  {
...
SAML.prototype.processValidlySignedAssertion = function(xml, inResponseTo, callback) {
  ...
  var parserConfig = {
    ...
    var finishProcessingValidlySignedAssertion = function(subjectConfirmation, confirmData, assertion, profile, nowMs){
      ...
      if (subjectConfirmation) {
        if (confirmData && confirmData.$) {
          ...
          if (inResponseTo && subjectInResponseTo && subjectInResponseTo != inResponseTo) {
            self.cacheProvider.remove(inResponseTo, function(err, key){
                if(err){
                    throw new Error('Unable to remove from cacheProvider: ' + err);
            }
            });
              return callback(new Error('InResponseTo is not valid'), null, false);
            }
              ...
              self.cacheProvider.get(subjectInResponseTo, function (err, value) {
                  var foundValidInResponseTo = false;

                  if (err) {
                      return callback(new Error('Unable to retrieve inResponseTo from cacheProvider: ' + err), null, false);
      }
                  ...
                  // Continue processing valid assertions...
                  self.cacheProvider.remove(inResponseTo, function(err, key){
                      if(err){
                          throw new Error('Unable to remove from cacheProvider: ' + err);
                      }
                  ...
                  return finishProcessingValidlySignedAssertion(subjectConfirmation, confirmData, assertion, profile, nowMs);
              });
          }
      }
    }
